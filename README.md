...
**Markdown 圖表與公式**
- Mermaid 圖表：` ```mermaid ` 區塊自動繪製成向量圖（流程圖、序列圖、甘特圖等），深淺色主題會跟著重繪
- 數學公式：`$$…$$` 以 KaTeX 排版
- Mermaid 元件約 3.5 MB，只有文件真的含圖表時才下載；語法錯誤或載入失敗會退回顯示原始碼並標示原因

**輸出 PDF**（TXT / MD）
- 按頂欄「輸出 PDF」開啟列印對話框，目的地選「另存為 PDF」
- 輸出的是可選取、可搜尋的向量文字（不是截圖），Mermaid 圖表與 KaTeX 公式一併保留為向量
- 自動轉成白底黑字的紙本版面（畫面在深色模式時也一樣，圖表會先重繪成淺色）
- 螢光筆底色一併印出；標題不落單、圖表與表格不被切成兩半
- 預設檔名取自開啟的檔名；PDF 與 EPUB 不支援（PDF 本身已是 PDF，EPUB 為 iframe 分頁）
...
- 需要網路連線載入 CDN 函式庫（pdf.js、epub.js、marked、KaTeX、Mermaid、OpenCC 等）
